	...
	public SecurityFilterChain authorizeRequests(HttpSecurity http) throws Exception {
		...
		http.cors(Customizer.withDefaults()) // Enable CORS
				...
				.authorizeHttpRequests(request -> request
						.requestMatchers("/users/register", "/v*/api-doc*/**", "/swagger-ui/**", "/users/signup",
								"/users/signin", "/admin/registerstaff")
						.permitAll()
						// required explicitly for JS clients (eg React app - to permit pre flight
						// requests)
						.requestMatchers(HttpMethod.OPTIONS).permitAll().requestMatchers("/doctor/**").hasRole("DOCTOR")
						.requestMatchers("/petowner/**", "/pet/**").hasRole("PETOWNER")
						.requestMatchers("/products/add", "/admin/**").hasRole("ADMIN")
