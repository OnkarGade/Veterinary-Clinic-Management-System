...
public class PetServiceImpl implements PetService {
	...
	public ApiResponse addPet(PetReqDto petReqDto) {
		System.out.println("Values of pet request dto - "+petReqDto);
		Authentication auth = SecurityContextHolder.getContext().getAuthentication();
		Long userId = (Long) auth.getCredentials();
		
		PetOwner po = petOwnerRepository.findByOwnerId(userId)
	...
	public List<PetRespDto> getAllPets() {
		return petRepository.findAll().stream().map(pet -> mapper.map(pet, PetRespDto.class))
